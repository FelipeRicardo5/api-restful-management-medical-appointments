...
module "ecr" {
  ...
  create          = true # staging owns the shared repository; production looks it up
}
...
# DATABASE_URL embeds the password, so it is stored as its own secret rather
# than assembled from parts inside the container.
...
module "ecs_service" {
  ...
  # Placeholder for the first `terraform apply`, before any image has been
  # built. .github/workflows/cd.yml replaces it with the real image via a
  # new task-definition revision on every deploy.
